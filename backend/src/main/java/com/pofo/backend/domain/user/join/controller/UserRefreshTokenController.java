...
//    }
    ...
    public ResponseEntity<RsData<TokenDto>> refreshToken(
        ...
        try {
            ...
            if (!tokenProvider.validateToken(refreshToken)) {
                log.warn("❌ 유효하지 않은 Refresh Token: {}", refreshToken);
                // 저장 시 사용한 키 규칙에 따라 subject를 추출해서 삭제
                String subject = tokenProvider.getEmailFromToken(refreshToken);
                String redisKey = "user_refresh_token:" + subject;
                redisTemplate.delete(redisKey); // ✅ 만료된 Refresh Token 삭제
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                        .body(new RsData<>("401", "Refresh Token이 유효하지 않습니다. 다시 로그인해주세요.", null));
            }
            ...
            // 4) 기존의 오래된 Refresh Token 삭제 (새로운 토큰으로 업데이트)
            String subject = tokenProvider.getEmailFromToken(refreshToken);
            String oldKey = "user_refresh_token:" + subject;
            redisTemplate.delete(oldKey);
            log.info("🗑️ 기존 Refresh Token 삭제: {}", refreshToken);

            // 5) 새로운 Refresh Token을 Redis에 저장 + 만료시간 설정 (초 단위 변환)
            long expireTimeSeconds = newTokenDto.getRefreshTokenValidationTime() / 1000; // ✅ 초 단위 변환
            // 저장 시에도 동일한 키 규칙 사용: subject를 통해 "user_refresh_token:" + subject
            String newKey = "user_refresh_token:" + tokenProvider.getEmailFromToken(newTokenDto.getRefreshToken());
            redisTemplate.opsForValue().set(
                    newKey,
                    newTokenDto.getRefreshToken(),
                    expireTimeSeconds,
